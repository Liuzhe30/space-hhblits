...
import math
max_range = 0
output_path = 'data/distance_2555_data/'
for uniprot_id in name_list:
    ...
    with open(pdb_path + uniprot_id + '.pdb') as pdb_file:
        ...
        distance_2555_matrix = np.zeros([hhm_seq_len, 2555], float)
        for i in range(hhm_seq_len):
            for j in range(hhm_seq_len):
                distance_2555_matrix[i][j] = math.sqrt(math.pow((pdb_matrix[i][0] - pdb_matrix[j][0]),2) + 
                                                  math.pow((pdb_matrix[i][1] - pdb_matrix[j][1]),2) + 
                                                  math.pow((pdb_matrix[i][2] - pdb_matrix[j][2]),2))
        print(distance_2555_matrix.shape)
        print(distance_2555_matrix)
        
        
    
    with open(output_path + uniprot_id + '.npy','w+') as out_file:
        np.savetxt(out_file, distance_2555_matrix, fmt='%.4f')
